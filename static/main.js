...
function logInfors(){
    ...
    const areas = text.split(', ').map(item => {
        ...
        return match ? { type: match[1].split(' ')[0], name: item.split(' ')[0] === 'Thị' ? match[1] : match[1].replace(/^(Xã|Phường) /, ''), district: match[2] } : null;
    }).filter(Boolean);
    ...
    const grouped = areas.reduce((acc, { type, name, district }) => {
        ...
        if (district === 'Sơn La' && type === 'Phường') acc[district].hasWard = true;
        return acc;
    }, {});
    
    let formattedText = '';
    for (let [district, { communes, hasWard }] of Object.entries(grouped)) {
        const label = hasWard ? 'các xã, phường' : 'các xã';
        formattedText += `- ${district}: ${label} ${communes.join(', ')}\n`;
    }
