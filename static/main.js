...
//             if(turf.intersect(feature,area)){
//                 intersectedProvinces.push(`${area.properties.NAME_2}`)
//             }
//         })
//     });
...
function logInfors(){
    ...
    drawnItemsDistricts.eachLayer(function(layer){
        ...
        data_districts.features.forEach(function(area){
            if(turf.intersect(feature,area)){
                intersectedDistricts.push(`${area.properties.TYPE_3} ${area.properties.NAME_3} (${area.properties.NAME_2})`)
            }
        })
    });
    text=[... new Set(intersectedDistricts)].join(", ")

    const areas = text.split(', ').map(item => {
        const match = item.match(/(.+) \((.+)\)/);
        return match 
            ? { 
                type: match[1].split(' ')[0], 
                name: item.split(' ')[0] === 'Thị' ? match[1] : match[1].replace(/^(Xã|Phường) /, ''), 
                district: match[2] } 
            : null;
    }).filter(Boolean); //có thể thay thế ngay trên
    console.log(areas) // return array
    
    const grouped = areas.reduce((acc, { type, name, district }) => {
        if (!acc[district]) acc[district] = { communes: [], hasWard: false };
        acc[district].communes.push(name);
        if (district === 'TP. Sơn La' && type === 'Phường') acc[district].hasWard = true;
        return acc;
    }, {});
    console.log(grouped)
    
    let formattedText = '';
    for (const [district, { communes, hasWard }] of Object.entries(grouped)) {
        const label = district === 'TP. Sơn La' ? 'các xã, phường' : communes.length === 1 ? 'xã' : hasWard ? 'các xã, phường' : 'các xã';
